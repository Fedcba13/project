...
public class DeliverManagePanel extends JPanel {
	...
	public DeliverManagePanel(MainFrame f) {
		...
		Object[][] rowData = getArr(payArr, null, null);
		...
		ActionListener ac = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (jr.getText().equals("전체 보기")) {
					obj = getArr(payArr, null, searchId.getText());
				} else if (jr.getText().equals("배송중")) {
					obj = getArr(payArr, "배송중", searchId.getText());
				} else {
					obj = getArr(payArr, "배송완료", searchId.getText());
				}
	...
	public Object[][] getArr(ArrayList<Payment> arr, String cate, String str) {

		ArrayList<Payment> tmp = new ArrayList<Payment>();

		if (cate == null) {
			tmp = (ArrayList<Payment>) arr.clone();
		} else {

			for (Payment pa : arr) {
				Calendar cal = Calendar.getInstance();
				cal.setTime(pa.getPayDate());
				cal.add(Calendar.DATE, pa.getDeliver());
				Date deliverDate = new Date(cal.getTimeInMillis());
		...
		Object rowData[][] = new Object[tmp.size()][columns.length];
		...
		for (int i = 0; i < tmp.size(); i++) {
			...
			Calendar cal = Calendar.getInstance();
			cal.setTime(tmp.get(i).getPayDate());
			cal.add(Calendar.DATE, tmp.get(i).getDeliver());
			Date deliverDate = new Date(cal.getTimeInMillis());

			Date curDate = new Date();

			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			rowData[i][5] = sdf.format(curDate);
			if (deliverDate.compareTo(curDate) < 0) {
				rowData[i][6] = "배송완료";
			} else {
				rowData[i][6] = "배송중";
