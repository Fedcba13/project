...
public class RegisterPanel extends JPanel {
	...
	public RegisterPanel(MainFrame f) {
		...
		sameIdCheck.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!valid("아이디", idTextField)) {// id가 빈값인지 확인
					return;
				} else {
		...
		registerBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!(valid("아이디", idTextField) && valid("이름", nameTextField) && valid("비밀번호", pwTextField)
						&& valid("비밀번호확인", repwTextField) && valid("생년월일", birthTextField)
						&& valid("핸드폰번호", phoneTextField) && valid("주소", addrTextField))) {
					return;
				} else if (!((birth.length() == 6) && isNum(birth))) {// 생년월일 6자리인지 확인, 전부 숫자인지 확인.
					focusMsg("생년월일 값이 올바르지 않습니다.", birthTextField);
					return;
				} else if (!((phone.length() == 11) && isNum(phone))) {// 핸드폰번호 11자리인지 확인, 전부 숫자인지 확인.
					focusMsg("핸드폰번호 값이 올바르지 않습니다.", phoneTextField);
					return;
					...
					focusMsg("비밀번호를 확인해 주세요.", repwTextField);
					return;
					...
					register.addCustomer(new Customer(id, name, pw, addr, date));
	...
	// false 시 focus
	public boolean valid(String s, JTextField j) {
		if (j.getText().equals("")) {
	...
	// focus + dialog띄우기
	public void focusMsg(String s, JTextField j) {
		j.requestFocus();
